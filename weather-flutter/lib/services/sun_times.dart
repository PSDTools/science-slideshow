...
/// Returns (sunrise, sunset) as fractional hours, or null for each if the sun
/// doesn't rise/set (polar extremes).
(double? sunrise, double? sunset) getSunTimes(double lat, double lon) {
  const toRad = pi / 180;
  const toDeg = 180 / pi;

  final now = DateTime.now();
  final jan1 = DateTime(now.year, 1, 1);
  final n = now.difference(jan1).inDays + 1; // day of year (1-based, ceil in original)
  final lngHour = lon / 15;
  final localOffsetHours = now.timeZoneOffset.inMinutes / 60.0;
  ...
  double? calc(bool isSunrise) {
    ...
    ra = (ra + ((l / 90).floor() * 90 - (ra / 90).floor() * 90)) / 15;
    ...
    final tResult = h + ra - 0.06571 * t - 6.622;
    return (((tResult - lngHour + localOffsetHours) % 24) + 24) % 24;
  }
